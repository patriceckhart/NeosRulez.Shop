...
//        $send_invoice = intval($args['cart_variables']['invoice']);
        ...
        if($this->settings['Mail']['debugMode']) {
            ...
            if(strpos($args['cart_variables']['recipient_mail'], ',') !== false) {
                ...
                if($send_mail) {
                    $this->sendInvoice($variables, $args['cart_variables']['order_subject'], $this->settings['Mail']['senderMail'], [$args['email'] => $args['firstname'] . ' ' . $args['lastname']], $args);
                    $this->sendInvoice($variables, $args['cart_variables']['order_subject'], [$args['email'] => $args['firstname'] . ' ' . $args['lastname']], $valid_recipient, $args);
                }
            } else {
                $this->send($variables, $args['cart_variables']['order_subject'], $this->settings['Mail']['senderMail'], [$args['email'] => $args['firstname'].' '.$args['lastname']], $args);
                ...
                if($send_mail) {
                    $this->sendInvoice($variables, $args['cart_variables']['order_subject'], $this->settings['Mail']['senderMail'], [$args['email'] => $args['firstname'] . ' ' . $args['lastname']], $args);
                    $this->sendInvoice($variables, $args['cart_variables']['order_subject'], [$args['email'] => $args['firstname'] . ' ' . $args['lastname']], [$args['cart_variables']['recipient_mail'] => $args['cart_variables']['recipient_mail']], $args);
                }
            }
        }
    }
    ...
    public function sendInvoice($variables, $subject, $sender, $recipient, $args) {
        $view = new \Neos\FluidAdaptor\View\StandaloneView();
...
//        $attachment = new \Swift_Attachment($pdf, $invoice_number . '_' . $this->translator->translateById('invoice', [], null, null, $sourceName = 'Main', $packageKey = 'NeosRulez.Shop') .'.pdf', 'application/pdf');
